...
class IdaeCarScraper:
    def __init__(self):
        ...
        self.engine = create_engine('sqlite:///idae_missing.db')
        self.df = self.load_or_create_db('cars')
        self.df_missing = pd.read_csv('not_in_titles.csv')
        self.cars_counter = 0
    ...
    def loop_cars(self, table_cars):
        for i, car in enumerate(table_cars):
            if self.df_missing['Nombre'].eq(car.text).any():  
                self.cars_counter += 1
                print(f"=== Page: {self.current_page} - Car: {self.cars_counter} ===")
                row = {}
    ...
    def load_or_create_db(self, table):
        if os.path.isfile('idae_missing.db') and self.engine.has_table(table):
